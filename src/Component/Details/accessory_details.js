...
const AccessoriesDetailsDisplay = () => {
    ...
    useEffect(() => {
        ...
        axios
        .get(`${base_url}/accessory_details?sub_category_id=${subcategoryId}&merchandise_id=${merchandiseId}`)
        .then((res) => {
          setAccessoriesDetails(res.data[0]);
        })
        .catch((error) => {
          console.log(error);
        });
    ...
    const proceed = () => {
        navigate(`/placeOrder/${accessoriesDetails.merchandise_name}`)
    }
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-lg-6">
                            <img src={accessoriesDetails.image} alt={accessoriesDetails.merchandise_name} className="img-fluid rounded-3 merchandise_image" />
                        </div>
                        <div className="col-lg-6 d-flex align-items-center">
                            <div className="large_button">
                                <h3 className="display-8 fw-bold custom_white">{accessoriesDetails.merchandise_name}</h3>
                                <p className="lead fw-bold custom_white">₹{accessoriesDetails.price}</p>
